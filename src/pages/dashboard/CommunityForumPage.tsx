...
export default function CommunityForumPage() {
  ...
  return (
    <div className="h-[calc(100vh-3.5rem)] flex flex-col md:flex-row gap-0 overflow-hidden relative">
      {/* Channel Sidebar */}
      ...
      <AnimatePresence>
        {showMembers && (
          <motion.div
            ...
            className="hidden lg:flex flex-col border-l border-border bg-muted/30 overflow-hidden"
          >
            <div className="p-3 border-b border-border flex items-center justify-between">
              <h3 className="text-sm font-bold text-foreground flex items-center gap-1.5">
                <Users className="w-4 h-4" /> Members
                <Badge variant="secondary" className="text-[10px] h-4 ml-1">{profiles.length}</Badge>
              </h3>
              <button onClick={() => setShowMembers(false)} className="p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-2">
              <div className="relative">
                <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
                <Input placeholder="Search members…" value={memberSearch} onChange={(e) => setMemberSearch(e.target.value)} className="h-8 text-xs pl-7" />
              </div>
            </div>
            <ScrollArea className="flex-1">
              <div className="p-2 space-y-1">
                {profiles
                  .filter(p => !memberSearch || p.full_name?.toLowerCase().includes(memberSearch.toLowerCase()) || p.expertise_tags?.some(t => t.toLowerCase().includes(memberSearch.toLowerCase())))
                  .map((profile) => (
                    <div key={profile.user_id} className="flex items-start gap-2.5 px-2.5 py-2 rounded-md hover:bg-muted/60 transition-colors">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-semibold text-foreground truncate">{profile.full_name || "Anonymous"}</p>
                        {profile.bio && (
                          <p className="text-[10px] text-muted-foreground line-clamp-1 mt-0.5">{profile.bio}</p>
                        )}
                        {profile.expertise_tags && profile.expertise_tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {profile.expertise_tags.slice(0, 3).map(tag => (
                              <Badge key={tag} variant="secondary" className="text-[9px] h-3.5 px-1.5">{tag}</Badge>
                            ))}
                            {profile.expertise_tags.length > 3 && (
                              <span className="text-[9px] text-muted-foreground">+{profile.expertise_tags.length - 3}</span>
                            )}
                          </div>
                        )}
                      </div>
                ...
                {profiles.filter(p => !memberSearch || p.full_name?.toLowerCase().includes(memberSearch.toLowerCase())).length === 0 && (
